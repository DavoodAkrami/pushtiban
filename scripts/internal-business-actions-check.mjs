...
assert.match(configuration, /side === "primary"/);
assert.match(configuration, /isSystemGeneratedCollectionField\(actionKey, field\)/);
assert.match(configuration, /roleMatches\.length === 1/);
assert.match(configuration, /\["check_availability", "create_reservation"\]\.includes\(actionKey\)/);
