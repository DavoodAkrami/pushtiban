...
//
// Receipt is durable before acknowledgement; the worker owns retries.
// ---------------------------------------------------------------------------
 ...
 */
export const instagramWebhookGet = async (request: NextRequest) => {
...
const mirrorConversationToTelegram = async ({
  ...
  for (const recipient of recipients) {
    const response = await telegramRequest(notifier.token, "sendMessage", {
...
export const instagramWebhookPost = async (request: NextRequest) => {
  ...
  try {
    ...
    for (const entry of payload.entry ?? []) {
      ...
      const events = [
        ...(entry.messaging ?? [])
          .filter((e) => !e.message?.is_echo && !e.message?.is_deleted)
          .map((event) => ({ kind: "message" as const, event })),
        ...(entry.changes ?? [])
          .filter((e) => e.field === "comments")
          .map((change) => ({ kind: "comment" as const, change })),
      ];
      for (const item of events) {
        const sender =
          item.kind === "message"
            ? item.event.sender?.id
            : item.change.value?.from?.id;
        const external =
          item.kind === "message"
            ? (item.event.message?.mid ??
              item.event.postback?.mid ??
              (item.event.timestamp
                ? `${sender}:${item.event.timestamp}:${item.event.postback?.payload ?? ""}`
                : null))
            : item.change.value?.id;
        if (!sender || !external) continue;
        const text =
          item.kind === "message"
            ? (item.event.message?.text ?? "")
            : (item.change.value?.text ?? "");
        const scope = conversationScope({
          userId: connection.user_id,
          channel: "instagram",
          connectionId: connection.id,
          customerExternalId: sender,
          conversationId: sender,
          deliveryId: external,
          customerMessage: text,
        });
        const privacy = await prepareReplay(scope, text);
        const normalized =
          item.kind === "message"
            ? {
                kind: item.kind,
                event: {
                  ...
                  postback: item.event.postback,
                  message: item.event.message
                    ? {
                        mid: item.event.message.mid,
                        text: text.slice(0, 4096),
                        quick_reply: item.event.message.quick_reply,
                        reply_to: item.event.message.reply_to?.story
                          ? {
                              story: {
                                id: item.event.message.reply_to.story.id,
                              },
                            }
                          : undefined,
                        attachments: item.event.message.attachments?.map(
                          (a) => ({ type: a.type }),
                        ),
                      }
                    : undefined,
            ...
            : {
                ...
                change: {
                  ...
                  value: {
                    ...
                    text: text.slice(0, 4096),
                    from: { id: sender },
                    media: item.change.value?.media,
                    parent_id: item.change.value?.parent_id,
                  },
                },
              };
        const persisted = structuredClone(normalized);
        if (persisted.event?.message)
          persisted.event.message.text = privacy.text;
        if (persisted.change?.value) persisted.change.value.text = privacy.text;
        const id = await receiveEvent({
          scope,
          externalId: `${item.kind}:${external}`.slice(0, 256),
          type: item.kind,
          payload: persisted,
          replayable: privacy.replayable,
        });
        after(() =>
          processEvent(id, () =>
            processInstagramEvent(connection.id, normalized),
          ),
        );
      }
    }
...
export const processInstagramEvent = async (
  connectionId: string,
  payload: { kind: string; event?: WebhookMessaging; change?: WebhookChange },
) => {
  const { data, error } = await createAdminClient()
    ...
    .maybeSingle();
  if (error) throw new ProcessingFailure("database_unavailable");
  if (!data) throw new ProcessingFailure("authorization_denied");
  const connection = data as Connection;
  const token = decryptSecret(connection.token_ciphertext);
  if (payload.kind === "message" && payload.event)
    await handleMessaging({ connection, event: payload.event, token });
  else if (payload.kind === "comment" && payload.change)
    await handleComment({ connection, change: payload.change, token });
};
